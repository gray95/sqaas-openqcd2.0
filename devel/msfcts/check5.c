...
static int ntm,i3d,tmin,tmax,dmax;
...
static void read_wflow_parms(void)
{
   ...
   error_root(ntm==0,1,"read_wflow_parms [check5.c]",
              "No flow times specified");
   error_root(eps<=0.0,1,"read_wflow_parms [check5.c]",
              "Step size eps must be positive");
   error_root((rule<0)||(rule>3),1,"read_wflow_parms [check5.c]",
   ...
   error(tms==NULL,1,"read_wflow_parms [check5.c]",
   ...
   if (my_rank==0)
   {
      ...
      error_root(ie!=0,1,"read_wflow_parms [check5.c]","Flow times must be "
...
static void check_files(void)
{
   ...
   else if (type&0x4)
   {
      ...
      error(name_size("%s/%d/%d/%sn%d_%d",cnfg_dir,nion-1,n-1,
            ...
            "check_files [check5.c]","cnfg_dir name is too long");
      sprintf(cnfg_dir,"%s/%d/%d",cnfg_dir,my_rank/n,my_rank%n);
      strcpy(cnfg_dir,line);
      check_dir(cnfg_dir);
   }
}
   ...
   for (icnfg=first;icnfg<=last;icnfg+=step)
   {
      ...
      for (i=0;i<ntm;i++)
      {
         ...
         for (j=0;j<=(tmax-tmin);j++)
         {
            ...
            {
               s[mu]=(tmin+j)/2;
               shift_fld(s,f[0],rf[0],f[2]);
               s[mu]-=(tmin+j);

               if (mu==0)
               {
                  shift_fld(s,f[0],rf[0],f[1]);
                  mul_flds(f[1],f[2]);
               }
               else
               {
                  shift_fld(s,f[0],rf[0],f[3]);
                  mul_flds(f[2],f[3]);
                  add_flds(f[1],f[2]);
               }
            ...
            convolute_flds(NULL,f[1],f[1],rf[0],rf[0],f[2]);
            mulr_fld(rv,f[2]);
