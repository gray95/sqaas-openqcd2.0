...
static double wt_scalar_dft4d(void)
{
   ...
   nflds=(int)((4*1024*1024)/(VOLUME*sizeof(complex_dble)))+1;
   if ((nflds%2)==1)
      nflds+=1;
...
static double wt_gluon_dft4d(void)
{
   ...
   nflds=(int)((4*1024*1024)/(8*VOLUME*sizeof(complex_dble)))+1;
   if ((nflds%2)==1)
      nflds+=1;
...
int main(int argc,char *argv[])
{
   ...
   if (my_rank==0)
   {
      ...
      printf("Time per lattice point: %4.3f usec\n\n",
             1.0e6*wdt/(double)(VOLUME));
   }
