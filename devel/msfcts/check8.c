...
static double square_norm(double *f)
{
   ...
   rqsm=vnorm_square_dble(VOLUME/2,1,(complex_dble*)(f));
...
int main(int argc,char *argv[])
{
   ...
   stdint_t l[13];
   char cnfg_dir[NAME_SIZE],blk_dir[NAME_SIZE];
   ...
   if ((cpr[0]==0)&&(cpr[1]==0)&&(cpr[2]==0)&&(cpr[3]==0))
      check_dir(blk_dir);
   error(name_size("%s/0/0/test.fld_b0",blk_dir)>=NAME_SIZE,1,
         "main [check8.c]","block_dir name is too long");
   sprintf(flds,"%s/0/0/test.fld_b0",blk_dir);
   ...
   error(name_size("%s/%d/%d/test_b%d",blk_dir,nio_nodes-1,
